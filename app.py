...
# _Associate Object____________________________________________________________
...
class Venue(db.Model):
    ...
    def __repr__(self):
        return f"\n<Venue id: {self.id} name: {self.name} location_id: {self.location_id}>"
...
def search_venues():
    ...
    data = Venue.query.filter(db.func.lower(request.form.get(
        'search_term', '')) in db.func.lower(Venue.name)).order_by('name').all()
...
def show_venue(venue_id):
    ...
    if len(venue_data) > 0:
        ...
        for show in past:
            ...
            past_show['start_time'] = show.start_time
        ...
        for show in upcoming:
            ...
            upcoming_show['artist_image_link'] = Artist.query.filter(
                Artist.id == show.artist_id).first().image_link
            upcoming_show['start_time'] = show.start_time
            upcoming_shows.append(past_show)
        data['past_shows'] = past_shows
        data['upcoming_shows'] = upcoming_shows
        data['past_shows_count'] = len(past_shows)
        data['upcoming_shows_count'] = len(upcoming_shows)
    return render_template('pages/show_venue.html', venue=data) if isFound else redirect(url_for('not_found_error', error=404))
...
def search_artists():
    ...
    data = Artist.query(Artist.id, Artist.name).filter(db.func.lower(request.form.get(
        'search_term', '')) in db.func.lower(Artist.name)).order_by('name').all()
